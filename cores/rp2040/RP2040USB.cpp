...
const uint8_t *tud_descriptor_device_cb(void) {
  ...
  if (productId == USBD_PID) {
    // Need a multi-endpoint config which will require changing the PID to help
    // Windows not barf
...
int __USBGetMouseReportID() { return __USBInstallKeyboard ? 2 : 1; }
...
int __USBGetHIDInstanceIndexForRawHID() {
  bool hasHID =
      __USBInstallKeyboard || __USBInstallMouse || __USBInstallJoystick;
...
void __SetupDescHIDReport() {
  // allocate memory for the HID report descriptors. We don't use them, but need
  // the size here.
  uint8_t desc_hid_report_mouse[] = {
      TUD_HID_REPORT_DESC_MOUSE(HID_REPORT_ID(1))};
  uint8_t desc_hid_report_joystick[] = {
      TUD_HID_REPORT_DESC_GAMEPAD(HID_REPORT_ID(1))};
  uint8_t desc_hid_report_keyboard[] = {
  ...
  if (__hid_report) {
    ...
    if (__USBInstallKeyboard) {
      memcpy(__hid_report, desc_hid_report_keyboard,
             sizeof(desc_hid_report_keyboard));
    }

    // 2.) mouse descriptor, if necessary. Additional offset & new array is
    // necessary if there is a keyboard.
    if (__USBInstallMouse) {
      // determine if we need an offset (USB keyboard is installed)
      if (__USBInstallKeyboard) {
        uint8_t desc_local[] = {TUD_HID_REPORT_DESC_MOUSE(HID_REPORT_ID(2))};
        memcpy(__hid_report + sizeof(desc_hid_report_keyboard), desc_local,
               sizeof(desc_local));
      } else {
        memcpy(__hid_report, desc_hid_report_mouse,
               sizeof(desc_hid_report_mouse));
      }
    }

    // 3.) joystick descriptor. 2 additional checks are necessary for mouse
    // and/or keyboard
    if (__USBInstallJoystick) {
      uint8_t reportid = 1;
      int offset = 0;
      ...
      uint8_t desc_local[] = {
          TUD_HID_REPORT_DESC_GAMEPAD(HID_REPORT_ID(reportid))};
...
void __SetupDescHID2Report() {
  if (__USBInstallSecondHID_RawHID) {
    uint8_t desc_hid_report_genericHID[] = {
...
void __SetupUSBDescriptor() {
  if (!usbd_desc_cfg) {
    ...
    uint8_t cdc_desc[TUD_CDC_DESC_LEN] = {
        ...
        // In & Out address, size & polling interval
        TUD_CDC_DESCRIPTOR(itf_cdc, USBD_STR_CDC, USBD_CDC_EP_CMD,
    ...
    uint8_t hid_desc[TUD_HID_DESC_LEN] = {
        // Interface number, string index, protocol, report descriptor len, EP
        // In & Out address, size & polling interval
        TUD_HID_DESCRIPTOR(itf_hid, 0, HID_ITF_PROTOCOL_NONE, hid_report_len,
                           EPNUM_HID, CFG_TUD_HID_EP_BUFSIZE, 10)};

    uint8_t msd_desc[TUD_MSC_DESC_LEN] = {TUD_MSC_DESCRIPTOR(
        itf_msd, 0, USBD_MSC_EPOUT, USBD_MSC_EPIN, USBD_MSC_EPSIZE)};
    ...
    uint8_t hid2_desc[TUD_HID_INOUT_DESC_LEN] = {
        // Interface number, string index, protocol, report descriptor len, EP
        // In & Out address, size & polling interval
        TUD_HID_INOUT_DESCRIPTOR(itf_hid2, 0, HID_ITF_PROTOCOL_NONE,
                                 hid2_report_len, EPNUM_HID2_EPOUT,
                                 EPNUM_HID2_EPIN, CFG_TUD_HID_EP_BUFSIZE, 10)};

    int usbd_desc_len =
        TUD_CONFIG_DESC_LEN + (hasSerial ? sizeof(cdc_desc) : 0) +
    ...
    uint8_t tud_cfg_desc[TUD_CONFIG_DESC_LEN] = {
        ...
        TUD_CONFIG_DESCRIPTOR(1, interface_count, USBD_STR_0, usbd_desc_len,
...
void __USBSubscribeHIDSetReportCallback(__USBHIDSetReportCallbackFn fn) {
  if (__hid_set_report_callback_functions_count < 2) {
    __hid_set_report_callback_functions
        [__hid_set_report_callback_functions_count++] = fn;
  }
}
...
extern "C" int32_t tud_msc_write10_cb(uint8_t lun, uint32_t lba,
                                      ...
                                      uint32_t bufsize) __attribute__((weak));
extern "C" int32_t tud_msc_write10_cb(uint8_t lun, uint32_t lba,
                                      uint32_t offset, uint8_t *buffer,
                                      uint32_t bufsize) {
  (void)lun;
...
extern "C" void tud_msc_capacity_cb(uint8_t lun, uint32_t *block_count,
                                    uint16_t *block_size) __attribute__((weak));
extern "C" void tud_msc_capacity_cb(uint8_t lun, uint32_t *block_count,
                                    uint16_t *block_size) {
  (void)lun;
  *block_count = 0;
